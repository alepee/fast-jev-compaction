...
describe('decisions', () => {
  ...
  it('honours truncateHeadChars, including a zero head', () => {
    ...
    expect(kept[2]?.toolResults?.[0]?.text).toBe(
      `${original.slice(0, 50)}\n[keep-the-thread truncated ${total - 50} chars of this tool result; re-run the tool if needed]`,
    );
    ...
    expect(noHead[2]?.toolResults?.[0]?.text).toBe(
      `[keep-the-thread truncated ${total} chars of this tool result; re-run the tool if needed]`,
    );
  });
});
...
describe('compact', () => {
  it('resends the full state with every batch and merges the answers', async () => {
    ...
    const output = await compact(
      ...
      { preserveRecentMessages: 1, maxRequestTokens: stateTokens + 260 },
    );
